                        ...
                        <div class="row mb-3">
                            <div class="col-5">
                                ...
                                <div class="input-group">
                                    <select id="carTypeSelect" class="form-select" name="carType"
                                        required>
                                    ...
                                    <button class="btn btn-outline-secondary" type="button"
                                        data-bs-toggle="modal" data-bs-target="#newCarTypeModal">Add new
                                        <b>car type</b>
                                    </button>
                                </div>
                            </div>
                            <div class="col-2 d-flex justify-content-center p-0">
                                <span
                                    class="material-symbols-outlined align-self-center">compare_arrows</span>
                            </div>
                            <%-- Circuit Types List--%>
                                <div class="col-5">
                                    <jsp:useBean id="circuitTypeList" scope="request"
                                        type="java.util.List" />
                                    <div class="input-group">
                                        <select id="circuitTypeSelect" class="form-select"
                                            name="circuitType" required disabled>
                                            <option value="">Select circuit type</option>
                                            <c:forEach items="${circuitTypeList}" var="circuit"
                                                varStatus="loop">
                                                <option value="${circuit.name}">
                                                    <c:out value="${circuit.name}" />
                                                </option>
                                            </c:forEach>
                                        </select>
                                        <button class="btn btn-outline-secondary" type="button"
                                            data-bs-toggle="modal" data-bs-target="#newCircuitTypeModal"
                                            id="circuitTypeBtn" disabled>Add new <b>circuit type</b>
                                        </button>
                                    </div>
                                </div>
                        </div>
                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            <table id="associationTable"
                                class="table table-light table-bordered rounded-3 overflow-hidden">
                                <thead>
                                    <tr>
                                ...
                                <tbody id="associationBody">
                                    <c:forEach items="${cCSuitMap}" var="cCSuit" varStatus="loop">
                                        <tr>
                                            <td rowspan="<c:out value=" ${fn:length(cCSuit.value)}" />">
                                            <c:out value="${cCSuit.key}" />
                                            </td>
                                            <td>
                                        ...
                                        <c:forEach items="${cCSuit.value}" var="circuitType"
                                            varStatus="loop">
                                            <c:if test="${loop.index > 0}">
                                                <tr>
                                                    <td>
                                                        <c:out value="${circuitType}" />
                                                    </td>
                                                    <td class="deleteBtnCol">
                                                        <button carType="${cCSuit.key}"
